...
let handler = async (m, { isPrems }) => {
    let time = global.db.data.users[m.sender].lastclaim + 2592000000
  if (new Date - global.db.data.users[m.sender].lastclaim < 2592000000) throw `Anda sudah mengklaim, klaim bulanan ini\ntunggu selama ${msToTime(time - new Date())} lagi`
...
function msToTime(duration) {
  ...
  claim  = (claim < 10) ? "0" + claim : claim
  hours = (hours < 10) ? "0" + hours : hours
  minutes = (minutes < 10) ? "0" + minutes : minutes
  seconds = (seconds < 10) ? "0" + seconds : seconds

  return claim + " hari " +  hours + " jam " + minutes + " menit"
}